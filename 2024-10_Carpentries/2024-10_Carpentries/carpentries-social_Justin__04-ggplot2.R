...
percent_society <- interviews_plotting %>%
    filter('NA'!=memb_assoc) %>%
    count(village, memb_assoc) %>%
    group_by(village) %>%
    mutate(percent = n / sum(n) *100) %>% 
    ungroup()

ggplot(percent_society, aes(x = village, y = percent, fill = memb_assoc)) +
    geom_bar(stat = "identity", color='black')+theme_bw()+
    # now set colors to black and white with black outline
...
# a title.
plt = ggplot(percent_wall_type, aes(x = village, y = percent, fill = respondent_wall_type)) +
    geom_bar(stat = "identity", position = "dodge") +
    ylab("Percent") +
    xlab("Wall Type") +
    ggtitle("Proportion of wall type by village")+theme_bw()
plt

# write a theme that replaces all font sizes with Times new roman, size 6
theme_new <- theme(
